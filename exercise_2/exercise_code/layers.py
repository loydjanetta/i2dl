...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    Returns a tuple of:
    - out: of shape (N, D)
    - cache: A tuple of values needed in the backward Passing
    """
    mode = bn_param['mode']
...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dsample_var = np.sum(dx_hat * x_minus_mean * (-0.5) * \
                          (var + eps) ** (-1.5), axis=0)
    dsample_mean = np.sum(dx_hat * (- ivar), axis=0) + \
                    dsample_var * (np.sum(-2 * x_minus_mean) / N)
    dx = dx_hat * ivar + dsample_var * (2 * x_minus_mean / N) + dsample_mean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1 / N) * ivar * (N * dx_hat - np.sum(dx_hat, axis=0) - x_norm * np.sum(dx_hat * x_norm, axis=0))
    dbeta = np.sum(dout, axis=0)

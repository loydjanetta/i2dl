...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(0, self.num_layers):
            ...
            self.params[w_key] = np.random.normal(scale=weight_scale, size=(layer_dims[i], layer_dims[i+1]))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            beta_key = 'beta' + str(i)
            batchnorm_key = 'batchnorm' + str(i)
            dropout_key = 'dropout' + str(i)
            cache_key = 'cache' + str(i)

            if i == self.num_layers:
                scores, cache = affine_forward(scores, self.params[w_key], self.params[b_key])
            else:
                if self.use_batchnorm:
                    scores, self.batchnorm_cache[batchnorm_key] = batchnorm_forward(scores.reshape(scores.shape[0], -1), 
                        self.params[gamma_key], self.params[beta_key], self.bn_params[i-1])
                scores, cache = affine_relu_forward(scores, self.params[w_key], self.params[b_key])
                if self.use_dropout:
                    scores, self.dropout_cache[dropout_key] = dropout_forward(scores, self.dropout_param)
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                dscores, grads[w_key], grads[b_key] = affine_backward(dscores, self.cache[cache_key])
            else:
                if self.use_dropout:
                    dscores = dropout_backward(dscores, self.dropout_cache[dropout_key])

                dscores, grads[w_key], grads[b_key] = affine_relu_backward(dscores, self.cache[cache_key])

                if self.use_batchnorm:
                    dscores, grads[gamma_key], grads[beta_key] = batchnorm_backward(dscores, self.batchnorm_cache[batchnorm_key])
